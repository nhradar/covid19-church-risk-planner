...
event_size <<- c(10, 20, 30, 40, 50, 60)
#event_size = c(50)
...
for ( asc_bias in asc_bias_list ){
  ...
  if (dim(data_Nr)[1] > 2000){
    ...
    for ( size in event_size ){
      ...
      maps[[size]] = map
      maps[[size]]$dependencies[[1]]$src[1] = "/srv/shiny-server/COVID19-Event-Risk-Planner/map_data/"
      mapshot(map, url = file.path('/srv/shiny-server/COVID19-Event-Risk-Planner/www', paste0(asc_bias,'_', size,'.html')))
    } 
    ...
    print(file.path('./daily_risk_map', current_time, 'asc_10_size_100.png'))
    
    
  }
#We don't need to generate images to tweet out. So save some cycles--NW
...
#      })
#  
#  map$dependencies[[1]]$src[1] = "/srv/shiny-server/COVID19-Event-Risk-Planner/map_data/"
# print("Map to png" )
